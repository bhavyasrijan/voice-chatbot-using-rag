...
if audio_bytes and not st.session_state.stop_audio:
...
if st.session_state.messages[-1]["role"] != "assistant" and not st.session_state.stop_audio:
    with st.chat_message("assistant"):
        with st.spinner("Thinking🤔..."):
            response = query_engine.query(st.session_state.messages[-1]["content_eng"])  # LLM response
