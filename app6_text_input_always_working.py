...
# Handle text input
if user_input and st.session_state.input_type != "audio" and not st.session_state.stop_audio:
...
# Handle audio input
if audio_bytes and st.session_state.input_type != "text" and not st.session_state.stop_audio:
...
# Process the user's query (regardless of input type)
if st.session_state.user_query_eng and st.session_state.messages[-1]["role"] == "user" and not st.session_state.stop_audio:
    ...
    # Reset user input and audio input to be ready for the next question
    st.session_state.user_query_eng = ""
    st.session_state.user_query_span = ""
    st.session_state.input_type = None
